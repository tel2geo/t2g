...
//var VocalR_obj = null;
			...
			// Suivant les standards unicode 6.1 
			// La plage de caractère arabe se situe entre les valeurs suivantes
			...
			function isArabic(textinarabic) {
				var pattern = /[\u0600-\u06ff]|[\u0750-\u077f]|[\ufb50-\ufbc1]|[\ufbd3-\ufd3f]|[\ufd50-\ufd8f]|[\ufd92-\ufdc7]|[\ufe70-\ufefc]|[\uFDF0-\uFDFD]/;
				resultar = pattern.test(textinarabic);
				return resultar;
			}
			function VocalR_recognize(commandesvocales){
					var laphrase = commandesvocales;
					if(isArabic(laphrase)){
						var laphrase = laphrase.trim();
						} else {
					...
					var laphrase = laphrasetrim.replace(/[^a-zA-Z 0-9]+/g,'');	
					...
					for(var key in VocalR_obj_functions) {
						...
						if (typeof val === "string" ){
								...
								var termearechercher = motRegExp.test(laphrase);
								...
								if(termearechercher){
									...
									console.log(val + " est contenu dans la phrase : " + laphrase +", on exécute la fonction : "+fonction_a_executer);
					...
					for(var key in VocalR_obj_functions.VocalR_obj_functions_S) {
						...
						// alert("Key VocalR_obj_functions.VocalR_obj_functions_S: "+key+" value:"+VocalR_obj_functions.VocalR_obj_functions_S[valtst]);
								...
								var segmentemotcle = laphrase.split(val_S);
								var termearechercher_S = motRegExp.test(laphrase);
								...
								if(typeof segmentemotcle[1] !== 'undefined' && segmentemotcle[1] !== ''){
									...
									var fonction_a_executer = key.toString(); 
									console.log(val_S + " est dans la phrase : " + laphrase +", on exécute la fonction : "+fonction_a_executer+", searchtag="+searchtag);
									// Retrouve l'objet fontion dans la page
									var trouve_fonction = window[fonction_a_executer];
									// Dernière vérification si c'est un objet ou une fonction
									// Execute la fonction
